...
# saving class mapping for Step 5
...
# Train the model (FULL 25 epochs)
history = model.fit(
    train_generator,
    epochs=25,
...
model.save('sultan_model.keras')
pd.DataFrame(history.history).to_csv('sultan_history.csv', index=False)
...
plt.xlabel('Epochs'); plt.ylabel('Accuracy'); plt.title('Model Accuracy'); plt.legend()

# Loss plot
plt.subplot(1, 2, 2)
plt.plot(history.history['loss'], label='Training Loss')
plt.plot(history.history['val_loss'], label='Validation Loss')
plt.xlabel('Epochs'); plt.ylabel('Loss'); plt.title('Model Loss'); plt.legend()

plt.tight_layout()
plt.savefig('sultan_training_curves.png', dpi=150)
plt.show()